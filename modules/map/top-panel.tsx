...
import { Eye, Rocket, Layers2Icon, InfoIcon } from "lucide-react";
...
export default function TopPanel() {
  ...
  return (
    <section className="absolute flex flex-row top-2 right-3 gap-2">
      <Select
        value={MODE_OPTIONS.find((item) => item.value === mode)?.label}
        onValueChange={(v) => {
      ...
      <Select
        value={LAPISAN_OPTIONS.find((item) => item.value == layer)?.label}
            ...
            {LAPISAN_OPTIONS.map((item) => {
              return (
                <SelectItem
                  key={item.value}
                  value={item.value}
                  className={"text-body-3 font-normal"}
                >
                  {item.label}
                </SelectItem>
              );
